 ...
 * </ol>
 *
 * <p><b>혜택 개정 추적은 스코프 밖이다</b>(사용자 결정 2026-08-10). 진짜 핀테크라면 필수지만
 * (공시 URL 해시 감시 → 재파싱 → 검수 큐) 여기까지는 가지 않는다. 그래서 카드 정보는
 * <b>수집 시점에 고정된 스냅샷</b>이고 시간이 지나면 낡는다 — 자동화 대신 <b>수집 기준일을
 * 화면에 병기</b>해서 "이 시점 공시 기준"임을 밝히는 것으로 처리한다. 비교만 하고 가입으로
 * 넘기지 않으므로 낡은 값이 곧바로 계약 손해가 되지는 않는다. <b>③ CTA 금지가 이 방어의
