 ...
 * 상품을 표에서 읽는다({@code V36}).
 *
 * <p><b>이 화면이 어디까지 하는지는 약관 정본이 정한다</b>({@code legal/terms-of-service.md}
 * 제7조). 운영사는 금융상품 계약의 대리·중개 <b>권한이 있다</b>고 밝히면서, 그 권한이 있음에도
 * <b>직접적인 판매 대리·중개를 수행하지 않고 정보 제공만 한다</b>고 함께 밝혔다. 조문이
 * 신용카드 추천을 예로 들어 이름까지 적고 있다.
 *
 * <p>그래서 이 클래스는 <b>혜택 비교까지</b>다. 지금 카드사로 넘어가는 신청 버튼이 없는 것은
 * 그 선언을 코드가 지키는 모습이고, 순위의 근거를 같은 응답에 싣는 것도 같은 이유다.
 * 그 선을 넘길 일이 생기면 <b>약관 제7조를 먼저 보고</b> 정한다 — 여기 주석이 정할 일이 아니다.
 *
 * <p><b>혜택 개정 추적은 스코프 밖이다</b>(사용자 결정 2026-08-10). 그래서 카드 정보는
 * <b>수집 시점에 고정된 스냅샷</b>이고 시간이 지나면 낡는다 — {@link Offer#asOf} 를 화면에
 * 병기해 "이 시점 공시 기준"임을 밝히는 것으로 처리한다.
 *
