 ...
 *
 * <p>그래서 {@code finntech.crypto.required=false} 로 끌 수 있게 하되 <b>기본값은 켬</b>이고,
...
public class CryptoRequiredGuard implements InitializingBean {
    ...
    public CryptoRequiredGuard(FieldCrypto crypto,
                               @Value("${finntech.crypto.required:true}") boolean required) {
        this.crypto = crypto;
    ...
    public void afterPropertiesSet() {
        ...
        if (!required) {
            log.warn("""
                    ...
                    (finntech.crypto.required=false 로 가드를 껐다).
        ...
        throw new IllegalStateException("""
                ...
                (CI 처럼 KMS 가 없는 곳에서만 finntech.crypto.required=false 로 건너뛴다.)""");
    }
}
