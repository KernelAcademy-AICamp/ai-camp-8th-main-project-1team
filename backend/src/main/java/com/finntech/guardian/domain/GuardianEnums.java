...
public final class GuardianEnums {
     ...
     * 돈 때문에 예산을 넘긴다.
     *
     * <p>이 값은 <b>시스템이 정하는 판정 대상 여부</b>다. 사용자가 남기는 소비 맥락은 분류
     * 확정({@code classifyPending})이 받으며, 그쪽은 차감을 바꾸지 않는다.
     */
    public enum TxKind {
    ...
    // 소비 맥락 칩(SpendChip)은 v1.5 초안에만 있었다. 붙이는 API도 읽는 화면도 없었고, 세 번째
    // 칩 '불가피한'은 미결(D29)로 남아 있었다 — 누르면 무제한으로 차감이 되돌려지는데, 뽑기로
    // 면제권을 주면서 상시 무료 되돌리기도 있으면 모순이라서다. 그 미결이 풀리기 전까지 칩은
    // 절반짜리 개념이라 걷어냈다. 되살릴 때는 D29부터 정한다. (2026-08-06)
