...
class ProfileBuilderTest {
    ...
    void indexIsSumOfContributions() {
        ...
        for (int i = 0; i < 4; i++) w.add(tx(LocalDateTime.of(2026, 2, 3 + i, 8, 0), "5622", "카페", 5000));   // removable 20,000, 아침
        w.add(tx(LocalDateTime.of(2026, 2, 10, 12, 0), "5611", "한식", 10000));
        w.add(tx(LocalDateTime.of(2026, 2, 11, 12, 0), "5611", "한식", 10000));                                 // 식비 합 40,000
        w.add(tx(LocalDateTime.of(2026, 2, 12, 9, 0), "6122", "통신비", 60000));                              // 고정비 60,000 → 최대
        ...
        assertEquals("통신비", p.topCategory1());
    ...
    void nightImpulseCapturesNightAndRoutineWaste() {
        List<UserPayment> w = List.of(
                tx(LocalDateTime.of(2026, 2, 5, 23, 0), "5619", "배달", 10000),   // 심야, removable
                tx(LocalDateTime.of(2026, 2, 6, 23, 0), "5619", "배달", 10000));  // 심야, removable  → 총 20,000
        SpendingPattern pattern = PatternAnalyzer.analyzeFrom(
                w, LocalDateTime.of(2026, 2, 1, 0, 0), LocalDateTime.of(2026, 3, 1, 0, 0), daypart);
