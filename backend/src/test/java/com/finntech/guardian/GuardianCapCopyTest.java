...
/**
 * <b>한도 알림은 합계를 말한다</b> — 카테고리를 "한도"에 붙이지 않는다.
 *
 * <p>C3·C6이 쓰는 {@code cap}·{@code remaining} 은 <b>챌린지 합계</b> 기준인데
 * (`GuardianRules.computeSnapshot`), 예전 문안은 "<b>식비</b> 한도의 80%예요"라고 말했다.
 * 거기 붙은 카테고리는 <b>방금 결제의 꼬리표</b>일 뿐이라 한도와 아무 상관이 없었다.
 *
 * <p>§8-T로 카테고리별 한도가 <b>실제로 생긴 뒤</b>로는 이 문장이 명백히 틀린 말이 된다 —
 * 홈이 "식비 198%"를 보여주는데 알림은 "식비 한도의 80%"라고 하면 두 숫자가 정면충돌한다.
...
class GuardianCapCopyTest {
    ...
    private static Map<String, Object> vars(String topCategory) {
        ...
        v.put("category", "식비");          // 방금 결제한 거래의 꼬리표 — 한도와 무관하다
        if (topCategory != null) v.put("topCategory", topCategory);
        return v;
    }

    @Test
    @DisplayName("C3 본문이 카테고리를 한도에 붙이지 않는다")
    void C3_한도는_합계다() {
        String body = GuardianCopy.fallback("C3", vars(null));
        assertThat(body).contains("한도의 80%");
        assertThat(body).doesNotContain("식비 한도");   // 옛 문안이 여기서 걸린다
    }

    @Test
    @DisplayName("C6 본문이 카테고리를 한도에 붙이지 않는다")
    void C6_한도는_합계다() {
        String body = GuardianCopy.fallback("C6", vars(null));
        assertThat(body).contains("한도 844,081원을 넘었어요");
        assertThat(body).doesNotContain("식비 한도");
    }

    @Test
    @DisplayName("제목에도 카테고리를 안 붙인다 — 20자 제한 때문에 제목이 더 오해되기 쉽다")
    void 제목() {
        assertThat(GuardianCopy.fallbackTitle("C3", vars(null))).isEqualTo("한도 80%");
        assertThat(GuardianCopy.fallbackTitle("C6", vars(null))).isEqualTo("한도 초과");
    }
    ...
    void 지목_문장() {
        ...
        assertThat(body).doesNotContain("쇼핑 한도");
    ...
    void 고정구() {
        assertThat(GuardianRules.stripFixedPhrases(
                java.util.List.of("에서 가장 많이 나갔어요", "한도의 80%")))
                .containsExactly("한도의 80%");
    }
