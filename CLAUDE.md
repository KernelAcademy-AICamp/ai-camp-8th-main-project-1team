...
1. **판단은 설명가능한 모델이, 표현은 AI가.** 판단은 규칙 엔진이거나 해석가능 ML(EBM)이며 **블랙박스는 쓰지 않는다**. AI(Gemini)에는 집계 수치만 보낸다(개별 소비 원문 금지). *(2026-07-29 개정 확정 — 리포트·소비건강점수가 ML 판정을 쓰게 되면서 조건 충족. 마스터 §4 원칙 1)*
