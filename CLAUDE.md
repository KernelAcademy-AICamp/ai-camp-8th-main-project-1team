...
- **소비 분류(§13-12)**: 실 명세서에는 **업종코드가 없다**. 그래서 ①확정 분류 사전(`merchant_category`) → ②업종코드 대조표 → ②-b 등록 업종 조회(사실) → ③LLM 추정 → ④카테고리없음/기타 순으로 답을 찾고, **순위는 `MerchantCategoryService` 한 곳에만** 둔다. ③은 **무료·유료를 가르지 않는다**(2026-08-21 개정) — 예전에는 무료를 '②-c 임시 분류(DB 미저장)'로 따로 뒀는데, 148건 실측에서 무료 1위(72.3%)가 유료 Gemini(70.5%)보다 높아 전제가 깨졌다. 둘 다 사전에 올라가고 출처는 `LLM` 하나다. **모델은 업종만 답하고 중분류는 우리 표가 정한다** — 프롬프트에 중분류를 보여주지 않는다(`IndustryPrompt`). 무료는 전부 **우선순위 큐**(`FreeChannelQueue`)를 거치고 유료는 **비상용**이다. **PG 번호는 키가 아니다** — 한 번호에 업종이 제각각인 가맹점이 붙으므로 번호를 버리고 이름으로 본다(읽기·쓰기 양쪽). 사전은 확정/추정 두 층이고 **추정은 판정에 참여하지 않는다**(원칙 1). 기준표는 `reference/D24_데이터분류기준표.md`.
