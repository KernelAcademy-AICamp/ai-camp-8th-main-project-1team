...
5. **규제**: 추천/판매 흐름의 금융상품은 **전부 더미**(금소법 회피) — 단 예외가 둘이다. ① **정보성 비교**: 무판매목적·무제휴·가입편의 없음이면 중개업 아님(금융위·금감원 유권해석 2022.6.15) → 실 금리 표시 허용(`SavingsCompareService`, "가입은 각 사로"). ② **카드는 실제 상품을 쓴다** *(2026-08-10 재개정 — 필수 기능이라 더미로는 성립하지 않는다)*. 카드에는 "더미라서 영업이 아니다"라는 다리가 없으므로 ①의 네 요건으로 선다. **넘으면 즉시 중개업 등록 대상이 되는 선 넷: 제휴 계약 · 수수료 수취 · 가입/신청 버튼(CTA) · 광고비 연동 순위.** 출처는 각 카드사 자사 **상품공시정보**(1차 원천). **혜택 개정 추적은 스코프 밖**이라 카드 정보는 수집 시점 스냅샷이고, 그래서 화면에 **수집 기준일을 반드시 병기**한다. 근거와 남는 위험은 마스터 §제외 항목의 🔁 개정(2026-08-10). 예적금·펀드·쇼핑 상품은 그대로 더미다.
