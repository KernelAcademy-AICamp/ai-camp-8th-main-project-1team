...
-- ## 왜 칸을 없애지 않고 값만 비우는가
--
-- 칸을 없애는 마이그레이션은 되돌릴 수 없다 — 롤백해도 스키마는 안 돌아와 옛 코드가 못 뜬다
-- (`guard-main` 이 그래서 막는다). 값만 비우면 **칸은 남고 내용만 사라진다.**
-- 목적은 "평문을 없애는 것"이지 "칸을 없애는 것"이 아니었다. 칸 정리는 한참 뒤 별도로 한다.
--
-- (이 주석에 그 SQL 낱말을 적지 않는 이유: `guard-main` 은 변경분의 낱말을 보고 판단하므로
--  설명에 적어도 걸린다. 실제로 이 파일이 그렇게 막혔다 — 2026-08-13.)
--
-- ## 조건이 이 파일의 전부다
--
