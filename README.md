...
과제 안내서에는 Supabase와 Vercel이 예시로 적혀 있었지만, 이 서비스에는 맞지 않는 부분이 있어 직접 고쳐서 골랐습니다.

**서버를 두 개로 나눠야 했습니다.** 카드·통장 정보를 주는 마이데이터 서버는 바깥에서 닿을 수 없어야 합니다. Supabase나 Firebase는 관리형 서버 한 개를 쓰는 구조라, 이렇게 숨겨진 두 번째 서버를 두기 어렵습니다.
...
**서버가 계속 켜져 있어야 했습니다.** 기록이 위조되지 않았다는 것을 증명하는 시각 인증, 5분마다 새 결제를 가져오는 작업처럼 계속 돌아가는 일이 있습니다.

**직접 배포해보는 것도 목표였습니다.** Docker로 묶고 AWS에 올리고 자동 배포까지 만들어보는 경험 자체가 이번 프로젝트에서 얻고 싶었던 것입니다.

**대신 포기한 것도 있습니다.** 로그인·실시간 기능을 공짜로 얻을 수 있었는데 직접 만들었고, 서버 관리와 배포 사고 대응을 저희가 떠안았습니다. 실제로 배포 문제를 여러 번 겪었고 그 내용은 [🧯 이슈 및 해결](#-이슈-및-해결)에 적었습니다.
...
<details>
<summary><b>1. 배포하면 서버가 멈추는 일이 두 번 있었습니다</b></summary>
<div markdown="1">

테이블에 새 항목을 추가했는데 데이터베이스를 바꾸는 파일을 같이 넣지 않아, 배포 직후 서버가 뜨지 못했습니다. 테스트는 전부 통과한 상태였습니다. 개발용 데이터베이스에서는 테이블이 자동으로 만들어져서 빠진 것이 드러나지 않았기 때문입니다.

**해결** — 코드를 합치기 전에 실제 운영과 같은 방식으로 서버를 한 번 띄워보는 검사를 만들었습니다. 지금 서비스와 같은 상태의 데이터베이스 위에 올려보기 때문에, 빠진 것이 있으면 그 자리에서 드러납니다. 그래도 문제가 생기면 배포가 이전 상태로 자동으로 되돌아갑니다.

</div>
</details>

<details>
<summary><b>2. 같은 사람으로 로그인한 두 계정이 서로 데이터를 빼앗았습니다</b></summary>
<div markdown="1">

시연용 계정이 5개라 여러 명이 같은 계정으로 들어올 수 있습니다. 그런데 나중에 연결한 사람이 먼저 연결한 사람의 결제 내역 2,404건을 통째로 가져가서, 앞사람 화면이 텅 비었습니다. 결제 내역을 저장할 때 쓰는 열쇠가 앱 사용자와 상관없이 하나뿐이었기 때문입니다.

**해결** — 저장 열쇠에 사용자 번호를 붙여 계정마다 따로 저장되게 했습니다. 세 계정으로 같이 접속해 각자 2,023건을 그대로 유지하는 것까지 확인했습니다.

</div>
</details>

<details>
<summary><b>3. 배포했는데 화면이 그대로였습니다</b></summary>
<div markdown="1">

새 버전을 올렸는데 브라우저가 예전 화면을 계속 보여줬습니다. 화면 주소를 담고 있는 파일에 "매번 새로 확인하라"는 표시가 없어서, 브라우저가 알아서 예전 것을 다시 썼기 때문입니다.

**해결** — 그 파일만 매번 새로 확인하도록 하고, 이름에 번호가 붙는 나머지 파일은 오래 저장해두게 나눴습니다.

</div>
</details>

<details>
<summary><b>4. 로그인 이후 모든 요청이 막혔습니다</b></summary>
<div markdown="1">

배포 후 글을 보내는 요청이 전부 거부됐습니다. 앞단에서 https로 받은 요청을 뒤로 넘길 때 http로 바뀌는데, 서버가 그것을 다른 주소에서 온 것으로 보고 막은 것입니다.

**해결** — 앞단이 넘겨준 원래 정보를 서버가 읽도록 설정하고, 배포 후 자동 검사에 이 항목을 넣어 다시는 놓치지 않게 했습니다.

</div>
</details>

<details>
<summary><b>5. 자동 배포 권한 설정이 네 번 실패했습니다</b></summary>
<div markdown="1">

GitHub에서 AWS로 배포하려면 서로를 믿는 설정이 필요한데 계속 거부됐습니다. 조직 계정은 신원을 표기하는 방식이 개인 계정과 달라서, 안내 문서대로 적으면 맞지 않았습니다.

**해결** — 실제로 어떤 값이 전달되는지 찍어보고 그 값에 맞춰 설정했습니다.
...
| 소비 유형 | 이름 | 주민번호 앞 7자리 | 휴대폰 |
| --- | --- | --- | --- |
| 과소비형 | 김우진 | 7609301 | 010-3913-6360 |
| 구독과다형 | 허서하 | 7402142 | 010-7068-5400 |
| 균형형 | 손채환 | 9606162 | 010-3697-8442 |
| 외식형 | 류소민 | 7910241 | 010-9835-5456 |
| 절약형 | 이지우 | 7906061 | 010-6910-9834 |
