...
- [MOA](#moa)
  ...
  - [💻 구동 방법](#-구동-방법)
  - [💾 결과물](#-결과물)
    - [중간 데모 (2026-08-07)](#중간-데모-2026-08-07)
    - [최종 발표 (2026-08-31)](#최종-발표-2026-08-31)
...
| 담당 | 하는 일 | 팀원 |
| :---: | :--- | :---: |
| ① 소비 분석 | 카드 내역을 카테고리로 나누고, 줄여볼 만한 항목을 찾아 근거와 함께 제안합니다 | **이상현** |
| ② 지킴·성장 | 지킬 돈을 정하고, 쓴 만큼 깎고, 남긴 만큼 캐릭터를 키웁니다 | **지윤정** |
| ③ 취향·추천 | 소비 성향을 읽어 리포트를 쓰고, 아낀 돈을 둘 통장을 비교해 보여줍니다 | **이가원** |
...
- **전부 아니면 전무가 아님** — 카테고리마다 얼마나 줄일지 강도를 정하고, 정한 예산을 넘긴 만큼만 깎입니다.
...
예산을 넘긴 만큼만 깎입니다. 깎이는 규칙은 이것 하나뿐이라, 왜 깎였는지 항상 설명할 수 있습니다.
...
3. 실제 사람 한 명의 카드 명세서도 같은 통로로 받습니다 (저장소 밖에 두고 넣습니다)
...
실제 카드 명세서에는 **업종코드가 없습니다.** 가맹점 이름만 보고 무엇을 산 건지 정해야 합니다.
그래서 네 단계로 순서대로 답을 찾습니다.

```text
① 확정 분류 사전   사람이 확인했거나 사용자가 직접 고친 것
② 업종코드 대조표   업종코드가 있으면 15개 중분류로 옮김
③ AI 추정         이름만 보고 짐작 — 표시만 하고 판정에는 쓰지 않음
④ 카테고리없음     여기까지 와도 모르면 모른다고 함
```

- **결제대행사(PG) 번호는 버립니다** — 한 번호에 서로 다른 가게가 붙어 있어 번호로 묶으면 오분류됩니다.
- **복합 사업자도 가릅니다** — 백화점 한 번호에 입점 브랜드가 여럿이면 이름까지 맞아야 인정합니다.
  택시처럼 상호가 수만 종이어도 전부 같은 업종인 곳과는 관측으로 구분합니다.
...
1. 판정 기록을 사슬로 이어 붙입니다 — 한 건을 고치면 뒤가 전부 어긋납니다
2. 바깥 시각 인증 기관(RFC 3161)에 하루치 요약을 맡겨 **언제 존재했는지**까지 남깁니다
3. `./scripts/demo-tamper.sh` 로 일부러 훼손해 보면 탐지되는 것을 볼 수 있습니다

💡 **접근성**:

빌드할 때마다 화면을 실제로 그려서 잽니다 — 색 대비, 터치 영역 크기, 초점 이동을
규칙이 아니라 **계산된 값**으로 확인합니다. (KWCAG 2.2 33개 항목)

---

## 🖥 서비스 화면

화면은 모두 35개입니다. 흐름별로 대표 화면을 둡니다.
...
본체 **94개**(컨트롤러 19개), 마이데이터 서버 **12개**입니다.
...
정의되지 않은 CSS 토큰, 명도대비 미달, KWCAG 위반, 타입 오류.
