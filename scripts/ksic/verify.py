...
그래서 사람이 눈으로 볼 표를 만들기 전에 기계가 잡을 수 있는 것부터 전부 잡는다.
  ...
  2. 모든 KSIC가 우리 중분류 16개 중 하나로 가는가
...
# 우리 소비 중분류. 2026-07-31 '금융/보험'을 더해 16개가 됐다 — 보험료가 '공과금'(전기업)에
# 얹혀 있어 한국전력공사가 보험료를 받고 있었다.
MIDS = ['식비', '카페/간식', '편의점/잡화', '대형마트', '술/유흥', '쇼핑', '취미/여가',
...
def main():
    ...
    # ── 2. 중분류 유효 ────────────────────────────────────────────────────
    badmid = [(s, l, m) for s, l, k, m in rows if m != 'DROP' and m not in MIDS]
    ...
    # 인허가 원천에 상호가 없고 브랜드만으로 채워지는 중분류. 보험사는 전국 9곳이 전부라
    # '동네 보험사'가 존재하지 않는다 — 독립 상호가 0인 것이 정상이다.
