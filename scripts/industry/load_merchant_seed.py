...
## 등록 업종도 같이 넣는다 (2026-08-25)

씨앗의 **3번째 칸이 등록 업종**인데 예전에는 이 로더가 그 칸을 안 읽었다. 그래서 이름 없는
씨앗 행은 브랜드를 맞출 문자열도 없고 업종 이름도 없어 **소분류를 얻을 길이 아예 없었다** —
운영에서 165행이 그 상태였다.

원천에 답이 있는데 안 옮긴 것뿐이라, 칸을 읽는 것만으로 **120행이 사실로 채워진다**
(나머지 46은 모호업종 18과 아래 '메모' 28이다).

**메모는 안 넣는다.** 수동 지정(`merchant-manual.tsv`)이 이 칸을 사람이 적는 근거 메모로
쓴다 — `올리브영 — KSNET·네이버페이 경유` 같은 문장이다. 사실 칸에 문장이 들어가면
`registryAnswered()` 가 *"조회를 이미 했다"* 로 읽어 진짜 조회를 막는다. 그래서 **대조표가
아는 업종명일 때만** 옮긴다(`industry-mid.json`).

소분류는 여기서 안 적는다. 업종 이름 → 소분류는 **표가 정하는 일**이라 자바 한 곳
(`SubCategorySweeper`)에 두고, 이 로더는 원재료만 나른다. 넣은 뒤 훑기를 부르면 찍힌다.
"""
import io
import os
import sys
...
MAP = os.path.join(ROOT, 'backend', 'src', 'main', 'resources', 'industry-mid.json')
INDUSTRY_MAX = 80            # merchant_category.registry_industry 의 길이


def known_industries():
    """대조표가 아는 업종 이름 — 씨앗 3번째 칸이 **사실인지 메모인지**를 가르는 잣대다.

    세 지도를 합친다. 소분류표(`subByIndustryName`)·중분류표(`midByIndustryName`)·
    국세청 세세분류 이름(`ntsByFineName`). 어디에도 없으면 업종명이 아니라 사람이 적은
    문장이므로 사실 칸에 넣지 않는다.
    """
    import json
    with io.open(MAP, encoding='utf-8') as f:
        d = json.load(f)
    return set(d['subByIndustryName']) | set(d['midByIndustryName']) | set(d['ntsByFineName'])
...
def rows(path, known):
    ...
    with io.open(path, encoding='utf-8') as f:
        for line in f:
            ...
            # 3번째 칸은 **등록 업종**이다. 단, 수동 지정 행은 여기에 사람이 적은 메모가
            # 들어 있어 대조표가 아는 이름일 때만 옮긴다 — 위 docstring 참고.
            ind = (c[2].strip() if len(c) > 2 else '')
            ind = ind[:INDUSTRY_MAX] if ind in known else ''
            ...
            if not c[1].strip():
                continue
            if name:
                out.append(('', c[1].strip(), name, codes, ind))
                continue
            if len(biz) != 10:
                continue
            out.append((biz, c[1].strip(), '', codes, ind))
    return out
...
def main():
    ...
    data = rows(src, known_industries())
    if not data:
        print(f'씨앗에 쓸 수 있는 줄이 없다: {src}', file=sys.stderr)
        sys.exit(1)

    dist = {}
    for _, m, _, _, _ in data:
        dist[m] = dist.get(m, 0) + 1
    byname = sum(1 for b, _, _, _, _ in data if not b)
    withcodes = sum(1 for _, _, _, c, _ in data if c)
    withind = sum(1 for _, _, _, _, i in data if i)
    ...
    # **0 도 정보다.** 등록 업종이 하나도 안 실리면 소분류가 통째로 안 붙는데 오류는 안 난다.
    print(f'   등록 업종을 든 행 {withind}곳 / {len(data)}'
          + ('  ⚠ 3번째 칸이 비었거나 전부 메모다' if not withind else ''))
    ...
    sql = """
        INSERT INTO merchant_category
            (business_number, merchant_name, category2, source, nts_codes, registry_industry,
             created_at, updated_at)
        VALUES (%s, %s, %s, %s, NULLIF(%s, ''), NULLIF(%s, ''), NOW(6), NOW(6))
        ON DUPLICATE KEY UPDATE
            category2  = IF(source = 'USER_CONFIRMED', category2, VALUES(category2)),
            -- **등록 업종은 비어 있을 때만 채운다.** 이미 있는 값은 살아 있는 조회처가
            -- 답한 것이라 씨앗보다 새롭다. 둘 다 사실이므로 다툴 일이 없고, 덮으면
            -- 새 값을 옛 값으로 되돌리는 일만 생긴다.
            registry_industry = IF(source = 'USER_CONFIRMED', registry_industry,
                                   IFNULL(NULLIF(registry_industry, ''),
                                          NULLIF(VALUES(registry_industry), ''))),
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.executemany(sql, [(b, n, m, SOURCE, c, i) for b, m, n, c, i in data])
            cur.execute("SELECT COUNT(*) FROM merchant_category")
