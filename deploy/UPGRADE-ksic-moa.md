...
### 알려진 잔여 — 이번 덤프에 남아 있는 것

**금액 배율 상한(`FLEX_SCALE_MAX=1.6`)은 코드에만 반영돼 있고 데이터에는 없다.**
수정 커밋(`8cf2585`) 이전에 생성한 데이터라, 수량 1건 결제의 **27.7%**가 카탈로그 가격
상한을 넘는다 — `클라이밍 1일권 181,000원`(정가 2~3만), `헬스 1개월 623,000원`(5~9만),
최대 14.2배. 월급이 큰 사용자일수록 심하다.

시연·판정에는 지장이 없지만(총액·비중·라벨은 일관됨) 개별 금액을 들여다보면 눈에 띈다.
**다음 재생성에서 자동으로 고쳐진다** — 그때 `PriceRangeTest`가 0.8% 미만인지 확인한다.
