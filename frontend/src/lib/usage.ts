...
const ENDPOINT = `${API_BASE}/api/usage/track`;
...
function flush(useBeacon = false) {
  ...
  const body = JSON.stringify(pending ? { events: batch, session: pending } : { events: batch });

  const settle = () => {
    if (!pending) return;
    const latest = session().state;
    // 그사이 세션이 바뀌었으면 남의 것을 지우지 않는다.
  ...
  if (useBeacon && typeof navigator.sendBeacon === 'function') {
    // 페이지가 닫히는 중에는 fetch 가 취소된다. sendBeacon 만 살아남는다.
    // 다만 헤더를 못 붙여 인증 토큰이 안 실린다 — 그래서 토큰을 질의 문자열로 준다.
